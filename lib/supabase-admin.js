...
export async function deleteUserProject(userId, projectId) {
  ...
  if (!uid || !pid) throw new Error('userId dan projectId wajib.');

  const url = `${SUPABASE_URL}/rest/v1/user_projects?user_id=eq.${encodeURIComponent(uid)}&id=eq.${encodeURIComponent(pid)}`;
  const res = await fetch(url, { method: 'DELETE', headers: adminHeaders() });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(body.message || body.error || 'Gagal hapus proyek cloud.');
  }
  return { ok: true };
}
